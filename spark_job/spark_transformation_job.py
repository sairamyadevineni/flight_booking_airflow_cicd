...
def main(env, bq_project, bq_dataset, transformed_table, route_insights_table, origin_insights_table):
    try:
        ...
        input_path = f"gs://us-central1-airflow-test-601a03c4-bucket/source-{env}"
        ...
        transformed_data = data.withColumn(
            "is_weekend", when(col("flight_day").isin("Sat", "Sun"), lit(1)).otherwise(lit(0)) # lit we use to put a constant value in that column
        ).withColumn(
            ...
            "booking_success_rate", expr("booking_complete / num_passengers") #expr we use for maths operations - division here 
        )
